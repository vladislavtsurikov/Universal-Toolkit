...
#pragma warning disable 0219
    public static class ColorUtils
    {
        ...
        public static float Hue(float r, float g, float b, bool factorize = false)
        {
            ...
            if (r >= g && g >= b)
            {
                hue = 60 * ((g - b) / (r - b)); //(A) If R ≥ G ≥ B  |  H = 60° x [(G-B)/(R-B)]
            }
            else if (g > r && r >= b)
            {
                hue = 60 * (2 - (r - b) / (g - b)); //(B) If G > R ≥ B  |  H = 60° x [2 - (R-B)/(G-B)]
            }
            else if (g >= b && b > r)
            {
                hue = 60 * (2 + (b - r) / (g - r)); //(C) If G ≥ B > R  |  H = 60° x [2 + (B-R)/(G-R)]
            }
            else if (b > g && g > r)
            {
                hue = 60 * (4 - (g - r) / (b - r)); //(D) If B > G > R  |  H = 60° x [4 - (G-R)/(B-R)]
            }
            else if (b > r && r >= g)
            {
                hue = 60 * (4 + (r - g) / (b - g)); //(E) If B > R ≥ G  |  H = 60° x [4 + (R-G)/(B-G)]
            }
            else if (r >= b && b > g)
            {
                hue = 60 * (6 - (b - g) / (r - g)); //(F) If R ≥ B > G  |  H = 60° x [6 - (B-G)/(R-G)]
            }

            if (factorize)
            {
                Mathf.RoundToInt(hue);
            }
        ...
        public static float RGBtoHUE(RGB value, bool factorize = false) => Hue(value.r, value.g, value.b, factorize);

        /// <summary> Convert pure HUE to RGB </summary>
        /// <param name="value"> HUE value </param>
        public static RGB HUEtoRGB(float value)
        {
            var R = Mathf.Abs(value * 6 - 3) - 1;
            var G = 2 - Mathf.Abs(value * 6 - 2);
            var B = 2 - Mathf.Abs(value * 6 - 4);
        ...
        public static Color RGBtoCOLOR(RGB value) => new(value.r, value.g, value.g);

        /// <summary> Convert HSL to Color </summary>
        /// <param name="value"> HSL value </param>
        public static Color HSLtoCOLOR(HSL value) => RGBtoCOLOR(value.ToRGB());

        /// <summary> Convert HSV to Color </summary>
        /// <param name="value"> HSV value </param>
        public static Color HSVtoCOLOR(HSV value) => RGBtoCOLOR(value.ToRGB());
        ...
        /// <param name="value"> RGB value </param>
        public static HSL RGBtoHSL(RGB value) => RGBtoHSL(value.r, value.g, value.b);

        /// <summary> Convert Color to HSL </summary>
        /// <param name="color"> Color value </param>
        public static HSL COLORtoHSL(Color color) => RGBtoHSL(color.r, color.g, color.b);
        ...
        /// <param name="value"> RGB value </param>
        public static HSV RGBtoHSV(RGB value) => RGBtoHSV(value.r, value.g, value.g);

        /// <summary> Convert Color to HSV </summary>
        /// <param name="color"> Color value </param>
        public static HSV COLORtoHSV(Color color) => RGBtoHSV(color.r, color.g, color.b);
        ...
        /// <param name="color"> Color value </param>
        public static RGB COLORtoRGB(Color color) => new(color.r, color.g, color.b);
