...
namespace OdinSerializer
{
    ...
    public abstract class BaseDictionaryKeyPathProvider<T> : IDictionaryKeyPathProvider<T>, IComparer<T>
    {
        ...
        int IDictionaryKeyPathProvider.Compare(object x, object y) => Compare((T)x, (T)y);

        object IDictionaryKeyPathProvider.GetKeyFromPathString(string pathStr) => GetKeyFromPathString(pathStr);

        string IDictionaryKeyPathProvider.GetPathStringFromKey(object key) => GetPathStringFromKey((T)key);
    }
}
