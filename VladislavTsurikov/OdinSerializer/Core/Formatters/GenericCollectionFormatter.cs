...
namespace OdinSerializer
{
    ...
    {
        /// <summary>
        ///     Determines whether the specified type can be formatted by a
        ///     <see cref="GenericCollectionFormatter{TCollection, TElement}" />.
        ///     <para />
        ///     The following criteria are checked: type implements <see cref="ICollection{T}" />, type is not abstract, type is
        ///     not a generic type definition, type is not an interface, type has a public parameterless constructor.
        /// </summary>
        /// <param name="type">The collection type to check.</param>
        /// <param name="elementType">The element type of the collection.</param>
        /// <returns>
        ///     <c>true</c> if the type can be formatted by a <see cref="GenericCollectionFormatter{TCollection, TElement}" />
        ///     , otherwise <c>false</c>
        /// </returns>
    ...
    /// <summary>
    ///     Formatter for all eligible types that implement the interface <see cref="ICollection{T}" />, and which have no
    ///     other formatters specified.
    ///     <para />
    ///     Eligibility for formatting by this class is determined by the
    ///     <see cref="GenericCollectionFormatter.CanFormat(Type, out Type)" /> method.
    /// </summary>
    /// <typeparam name="TCollection">The type of the collection.</typeparam>
    /// <typeparam name="TElement">The type of the element.</typeparam>
    public sealed class GenericCollectionFormatter<TCollection, TElement> : BaseFormatter<TCollection>
        where TCollection : ICollection<TElement>, new()
    {
        ...
        static GenericCollectionFormatter()
        {
            ...
            {
                throw new ArgumentException("Cannot treat the type " + typeof(TCollection).Name +
                                            " as a generic collection.");
            }

            if (e != typeof(TElement))
            {
                throw new ArgumentException("Type " + typeof(TElement).Name +
        ...
        /// </returns>
        protected override TCollection GetUninitializedObject() => new();
    ...
    public sealed class WeakGenericCollectionFormatter : WeakBaseFormatter
    {
        ...
        public WeakGenericCollectionFormatter(Type collectionType, Type elementType) : base(collectionType)
        {
            ...
            CountProperty = collectionType.GetProperty("Count",
                BindingFlags.Public | BindingFlags.Instance | BindingFlags.NonPublic);
            AddMethod = collectionType.GetMethod("Add",
                BindingFlags.Public | BindingFlags.Instance | BindingFlags.NonPublic, null, new[] { elementType },
                null);

            if (AddMethod == null)
            {
                throw new ArgumentException("Cannot treat the type " + collectionType.Name +
                                            " as a generic collection since it has no accessible Add method.");
            }

            if (CountProperty == null || CountProperty.PropertyType != typeof(int))
            {
                throw new ArgumentException("Cannot treat the type " + collectionType.Name +
                                            " as a generic collection since it has no accessible Count property.");
            }
            ...
            if (GenericCollectionFormatter.CanFormat(collectionType, out e) == false)
            {
                throw new ArgumentException(
                    "Cannot treat the type " + collectionType.Name + " as a generic collection.");
            }

            if (e != elementType)
            {
                throw new ArgumentException("Type " + elementType.Name +
                                            " is not the element type of the generic collection type " +
                                            collectionType.Name + ".");
            }
        }
