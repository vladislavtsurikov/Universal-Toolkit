    ...
    public class DelegateFormatter<T> : BaseFormatter<T> where T : class
    {
        ...
        protected override void DeserializeImplementation(ref T value, IDataReader reader)
        {
            ...
            if (invocationList != null)
            {
                ...
                catch (Exception ex)
                {
                    reader.Context.Config.DebugContext.LogError(
                        "Recombining delegate invocation list upon deserialization failed with an exception of type " +
                        ex.GetType().GetNiceFullName() + " with the message: " + ex.Message);
                }
                ...
                if (combinedDelegate != null)
                {
                    ...
                    {
                        reader.Context.Config.DebugContext.LogWarning("Could not cast recombined delegate of type " +
                                                                      combinedDelegate.GetType().GetNiceFullName() +
                                                                      " to expected delegate type " +
                                                                      DelegateType.GetNiceFullName() + ".");
                    }
                }
            ...
            if (methodInfo == null)
            {
                if (useSignature)
                {
                    reader.Context.Config.DebugContext.LogWarning("Could not find method with signature " + name + "(" +
                                                                  string.Join(", ",
                                                                      signature.Select(p => p.GetNiceFullName())
                                                                          .ToArray()) + ") on type '" +
                                                                  declaringType.FullName +
                                                                  (wasAmbiguous
                                                                      ? "; resolution was ambiguous between multiple methods"
                                                                      : string.Empty) + ".");
                }
                else
                {
                    reader.Context.Config.DebugContext.LogWarning("Could not find method with name " + name +
                                                                  " on type '" + declaringType.GetNiceFullName() +
                                                                  (wasAmbiguous
                                                                      ? "; resolution was ambiguous between multiple methods"
                                                                      : string.Empty) + ".");
                }
            ...
            if (methodInfo.IsGenericMethodDefinition)
            {
                ...
                {
                    reader.Context.Config.DebugContext.LogWarning("Method '" + declaringType.GetNiceFullName() + "." +
                ...
                var argCount = methodInfo.GetGenericArguments().Length;
                ...
                catch (Exception ex)
                {
                    reader.Context.Config.DebugContext.LogWarning("Method '" + declaringType.GetNiceFullName() + "." +
                                                                  ...
                                                                  string.Join(", ",
                                                                      genericArguments.Select(p => p.GetNiceFullName())
                                                                          .ToArray()) +
                                                                  "'. Method creation failed with an exception of type " +
                                                                  ex.GetType().GetNiceFullName() +
            ...
            if (value == null)
            {
                reader.Context.Config.DebugContext.LogWarning("Failed to create delegate of type " +
                                                              delegateType.GetNiceFullName() + " from method '" +
        ...
        protected override void SerializeImplementation(ref T value, IDataWriter writer)
        {
            ...
            {
                writer.Context.Config.DebugContext.LogError(
                    "Cannot serialize delegate made from dynamically emitted method " + methodInfo + ".");
                return;
            }
            ...
            {
                writer.Context.Config.DebugContext.LogError(
                    "Cannot serialize delegate made from the unresolved generic method definition " + methodInfo +
                    "; how did this even happen? It should not even be possible to have a delegate for a generic method definition that hasn't been turned into a generic method yet.");
                return;
            }
        ...
        ///     <see cref="M:OdinSerializer.BaseFormatter`1.RegisterReferenceID(`0,OdinSerializer.IDataReader)" /> and
        ///     <see
        ///         cref="M:OdinSerializer.BaseFormatter`1.InvokeOnDeserializingCallbacks(`0,OdinSerializer.DeserializationContext)" />
