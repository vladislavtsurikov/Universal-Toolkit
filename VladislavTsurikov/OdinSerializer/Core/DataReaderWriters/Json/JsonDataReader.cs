...
namespace OdinSerializer
{
    ...
    public class JsonDataReader : BaseDataReader
    {
        ...
        private readonly Dictionary<int, Type> seenTypes = new(16);
...
#pragma warning restore IDE0009 // Member access should be qualified.
        ...
        public override bool EnterNode(out Type type)
        {
            ...
            if (peekedEntryType == EntryType.StartOfNode)
            {
                ...
                if (peekedEntryName == JsonConfig.TYPE_SIG && peekedEntryContent != null &&
                    peekedEntryContent.Length > 0)
                {
                    if (peekedEntryType == EntryType.Integer)
                    {
                        ...
                        if (ReadInt32(out typeID))
                        {
                            ...
                            {
                                Context.Config.DebugContext.LogError("Missing type id for node with reference id " +
                                                                     id + ": " + typeID);
                            }
                        }
                        else
                        {
                            Context.Config.DebugContext.LogError("Failed to read type id for node with reference id " +
                                                                 id);
                            type = null;
                        }
                    }
                    ...
                    {
                        var typeNameStartIndex = 1;
                        var typeID = -1;
                        var idSplitIndex = peekedEntryContent.IndexOf('|');

                        if (idSplitIndex >= 0)
                        {
                            typeNameStartIndex = idSplitIndex + 1;
                            var idStr = peekedEntryContent.Substring(1, idSplitIndex - 1);

                            if (int.TryParse(idStr, NumberStyles.Any, CultureInfo.InvariantCulture, out typeID) ==
                                false)
                            {
                                typeID = -1;
                            }
                        ...
                        type = Context.Binder.BindToType(
                            peekedEntryContent.Substring(typeNameStartIndex,
                                peekedEntryContent.Length - (1 + typeNameStartIndex)), Context.Config.DebugContext);
        ...
        ///     <see cref="IDataReader.SkipEntry(DeserializationContext)" /> until an <see cref="EntryType.EndOfNode" /> is
        ///     reached, or the end of the stream is reached.
        ///     <para />
        ...
        public override bool EnterArray(out long length)
        {
            ...
            if (peekedEntryType == EntryType.StartOfArray)
            {
                ...
                if (int.TryParse(peekedEntryContent, NumberStyles.Any, CultureInfo.InvariantCulture, out intLength) ==
                    false)
                {
                ...
                {
                    Context.Config.DebugContext.LogError(
                        "Failed to find regular array content entry after array length entry!");
                    length = 0; // No array content for you!
                    return true;
                }
        ...
        ///     <see cref="IDataReader.SkipEntry(DeserializationContext)" /> until an <see cref="EntryType.EndOfArray" /> is
        ///     reached, or the end of the stream is reached.
        ///     <para />
        ...
        public override bool ReadPrimitiveArray<T>(out T[] array)
        {
            ...
            if (peekedEntryType == EntryType.PrimitiveArray)
            {
                ...
                {
                    Context.Config.DebugContext.LogError(
                        "Failed to find primitive array content entry after array length entry!");
                    array = null; // No array content for you!
                    return false;
                }
                ...
                var reader = (Func<T>)primitiveArrayReaders[typeof(T)];
        ...
        public override bool ReadExternalReference(out string id)
        {
            ...
            if (peekedEntryType == EntryType.ExternalReferenceByString)
            {
                ...
                {
                    id = id.Substring(JsonConfig.EXTERNAL_STRING_REF_SIG_FIXED.Length + 2,
                        id.Length - (JsonConfig.EXTERNAL_STRING_REF_SIG_FIXED.Length + 3));
                }
        ...
        private bool ReadAnyIntReference(out int value)
        {
            ...
            for (var i = 0; i < peekedEntryContent.Length; i++)
            {
            ...
            var idStr = peekedEntryContent.Substring(separatorIndex + 1);
