...
namespace OdinSerializer
{
    ...
    public abstract class BaseDataReaderWriter
    {
        ...
        /// </value>
        public bool IsInArrayNode => NodeDepth == 0 ? false : NodesArray[NodeDepth - 1].IsArray;
        ...
        /// </value>
        protected int NodeDepth { get; private set; }

        /// <summary>
        ///     Gets the current nodes array. The amount of nodes contained in it is stored in the <see cref="NodeDepth" />
        ...
        ///     The current node.
        /// </value>
        protected NodeInfo CurrentNode => NodeDepth == 0 ? NodeInfo.Empty : NodesArray[NodeDepth - 1];
        ...
        {
            if (NodeDepth == NodesArray.Length)
            {
                ExpandNodes();
            }

            NodesArray[NodeDepth] = node;
            NodeDepth++;
        }
        ...
        protected void PushNode(string name, int id, Type type)
        {
            ...
            NodesArray[NodeDepth] = new NodeInfo(name, id, type, false);
            NodeDepth++;
        }

        /// <summary>
        ///     Pushes an array node onto the node stack. This uses values from the current node to provide extra info about the
        ///     array node.
        /// </summary>
        protected void PushArray()
        {
            ...
            if (NodeDepth == 0 || NodesArray[NodeDepth - 1].IsArray)
            {
                NodesArray[NodeDepth] = new NodeInfo(null, -1, null, true);
            }
            else
            {
                NodeInfo current = NodesArray[NodeDepth - 1];
                NodesArray[NodeDepth] = new NodeInfo(current.Name, current.Id, current.Type, true);
            }
        ...
        {
            var newArr = new NodeInfo[NodesArray.Length * 2];

            NodeInfo[] oldNodes = NodesArray;
        ...
        protected void PopArray()
        {
            ...
            if (NodesArray[NodeDepth - 1].IsArray == false)
            {
        ...
        protected void ClearNodes() => NodeDepth = 0;
    }
}
