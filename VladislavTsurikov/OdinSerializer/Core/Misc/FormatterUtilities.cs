...
You probably need to assign the nullValue variable of the {0} script in the inspector.";
        ...
        private static readonly DoubleLookupDictionary<ISerializationPolicy, Type, Dictionary<string, MemberInfo>>
            MemberMapCache = new();

        private static readonly object LOCK = new();

        private static readonly HashSet<Type> PrimitiveArrayTypes = new(FastTypeComparer.Instance)
        {
            typeof(char),
            typeof(sbyte),
            typeof(short),
        ...
        static FormatterUtilities()
        {
            ...
            UnityObjectRuntimeErrorStringField =
                typeof(Object).GetField("m_UnityRuntimeErrorString", Flags.InstanceAnyVisibility);

            if (UnityObjectRuntimeErrorStringField == null)
            {
                Debug.LogWarning(
                    "A change in Unity has hindered the Serialization system's ability to create proper fake Unity null values; the UnityEngine.Object.m_UnityRuntimeErrorString field has been renamed or removed.");
            }
#endif
        }

        /// <summary>
        ///     Gets a map of all serializable members on the given type. This will also properly map names extracted from
        ///     <see cref="UnityEngine.Serialization.FormerlySerializedAsAttribute" /> and
        ///     <see cref="PreviouslySerializedAsAttribute" /> to their corresponding members.
        /// </summary>
        ...
        /// <summary>
        ///     Creates a fake Unity null value of a given type, for the given <see cref="UnityEngine.Object" />-derived owning
        ///     type.
        ///     <para />
        ...
        ///     Type of the owning value. This is the value which changes the
        ///     <see cref="MissingReferenceException" /> which you get.
        /// </param>
        ...
        public static Object CreateUnityNull(Type nullType, Type owningType)
        {
            ...
            var nullValue = (Object)FormatterServices.GetUninitializedObject(nullType);

            if (UnityObjectRuntimeErrorStringField != null)
            {
                UnityObjectRuntimeErrorStringField.SetValue(nullValue,
                    string.Format(CultureInfo.InvariantCulture, UnityObjectRuntimeErrorString, owningType.Name));
            }
        ...
        ///     <para />
        ///     The following criteria are checked: type.IsPrimitive or type.IsEnum, or type is a <see cref="decimal" />,
        ...
        /// <summary>
        ///     Determines whether a given type is a primitive array type. Namely, arrays with primitive array types as elements
        ///     are primitive arrays.
        ///     <para />
        ///     The following types are primitive array types: <see cref="char" />, <see cref="sbyte" />, <see cref="short" />,
        ///     <see cref="int" />, <see cref="long" />, <see cref="byte" />, <see cref="ushort" />, <see cref="uint" />,
        ///     <see cref="ulong" />, <see cref="decimal" />, <see cref="bool" />, <see cref="float" />, <see cref="double" /> and
        ///     <see cref="Guid" />.
        /// </summary>
        /// <param name="type">The type to check.</param>
        /// <returns><c>true</c> if the given type is a primitive array type; otherwise, <c>false</c>.</returns>
        public static bool IsPrimitiveArrayType(Type type) => PrimitiveArrayTypes.Contains(type);

        /// <summary>
        ///     Gets the type contained in the given <see cref="MemberInfo" />. Currently only <see cref="FieldInfo" /> and
        ...
        private static Dictionary<string, MemberInfo> FindSerializableMembersMap(Type type, ISerializationPolicy policy)
        {
            ...
            {
                IEnumerable<FormerlySerializedAsAttribute> serializedAsAttributes =
                    member.GetAttributes<FormerlySerializedAsAttribute>();
        ...
        private static void FindSerializableMembers(Type type, List<MemberInfo> members, ISerializationPolicy policy)
        {
            ...
            foreach (MemberInfo member in type.GetMembers(Flags).Where(n => n is FieldInfo || n is PropertyInfo))
            {
                ...
                {
                    var nameAlreadyExists = members.Any(n => n.Name == member.Name);
        ...
        /// <summary>
        ///     Gets an aliased version of a member, with the declaring type name included in the member name, so that there are no
        ///     conflicts with private fields and properties with the same name in different classes in the same inheritance
        ///     hierarchy.
        /// </summary>
        public static MemberInfo GetPrivateMemberAlias(MemberInfo member, string prefixString = null,
            string separatorString = null)
        {
            ...
            if (member is PropertyInfo)
            {
                if (separatorString != null)
                {
                    return new MemberAliasPropertyInfo(member as PropertyInfo,
                        prefixString ?? member.DeclaringType.Name, separatorString);
                }
