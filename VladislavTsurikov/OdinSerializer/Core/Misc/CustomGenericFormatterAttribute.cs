...
{
    /// <summary>
    ///     Attribute indicating that a generic type definition class which implements the <see cref="IFormatter{T}" />
    ///     interface somewhere in its hierarchy is a custom formatter for *any variation* of the generic type definition T.
    ///     <para />
    ///     The formatter's generic type parameters are mapped onto the serialized type's generic type parameters.
    ///     <para />
    ///     For example, <see cref="DictionaryFormatter{TKey, TValue}" /> implements <see cref="IFormatter{T}" />, where T is
    ...
    public class CustomGenericFormatterAttribute : CustomFormatterAttribute
    {
        ...
        ///     The type given in <paramref name="serializedGenericTypeDefinition" /> is not
            ...
            : base(priority)
        {
            ...
            {
                throw new ArgumentException("The type " + serializedGenericTypeDefinition.Name +
