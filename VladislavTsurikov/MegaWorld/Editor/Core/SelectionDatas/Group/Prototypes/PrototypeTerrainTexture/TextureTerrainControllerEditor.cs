...
{
	[ResourceController(typeof(Runtime.Core.SelectionDatas.Group.Prototypes.PrototypeTerrainTexture.PrototypeTerrainTexture))]
	public class TextureTerrainControllerEditor : ResourceControllerEditor
    {
		...
		private static readonly List<Layer> _paletteLayers = new List<Layer>();
					...
					case TextureTerrainResourcesController.TerrainResourcesSyncError.None:
					{
						...
						GUILayout.BeginHorizontal();
						{
							...
							GUILayout.BeginVertical();
							{
								...
								{
									TextureTerrainResourcesController.UpdatePrototypesFromTerrain(Terrain.activeTerrain, group);
								}
					...
					case TextureTerrainResourcesController.TerrainResourcesSyncError.MissingTerrainLayer:
					{
						...
						if (selectedPrototypes.Count == 1)
						{
							GUILayout.BeginHorizontal();
							{
								...
								GUILayout.BeginVertical();
								{
									_terrainLayerSettingsFoldout = CustomEditorGUILayout.Foldout(_terrainLayerSettingsFoldout, "Terrain Layer Settings");
		...
		{
			if (group.PrototypeType == typeof(Runtime.Core.SelectionDatas.Group.Prototypes.PrototypeTerrainTexture.PrototypeTerrainTexture))
			{
				TerrainResourcesController.DetectSyncError(group, Terrain.activeTerrain);
				
				if(TextureTerrainResourcesController.SyncError != TextureTerrainResourcesController.TerrainResourcesSyncError.None)
				{
					return true;
				}
			}
		...
		private static void UpdateLayerPalette(Terrain terrain)
		{
			...
			bool[] selectedList = new bool[_paletteLayers.Count];
			for(int i = 0; i < _paletteLayers.Count; i++)
			{
			...
			foreach (TerrainLayer layer in terrain.terrainData.terrainLayers)
			{
				if(layer != null) 
				{
					...
					{
						AssignedLayer = layer,
						Selected = selectedList.ElementAtOrDefault(index)
					};
		...
		private static void ChangeTerrainLayer(Runtime.Core.SelectionDatas.Group.Group group)
		{
			...
			if (selectedPrototypes.Count == 1)
			{
				Runtime.Core.SelectionDatas.Group.Prototypes.PrototypeTerrainTexture.PrototypeTerrainTexture prototypeTerrainTexture = (Runtime.Core.SelectionDatas.Group.Prototypes.PrototypeTerrainTexture.PrototypeTerrainTexture)selectedPrototypes[0];
				
				prototypeTerrainTexture.TerrainLayer = (TerrainLayer)CustomEditorGUILayout.ObjectField(new GUIContent("Terrain Layer"), prototypeTerrainTexture.TerrainLayer == null, prototypeTerrainTexture.TerrainLayer, typeof(TerrainLayer));

			}
		}

		private static void DrawSelectedWindowForTerrainTextureResources(Terrain terrain, Runtime.Core.SelectionDatas.Group.Group group)
		{
		...
		private static bool IsNecessaryToDrawIconsForPrototypes(Rect brushWindowRect, Color initialGUIColor, Terrain terrain, ref bool dragAndDrop)
		{
		...
		private static void DrawProtoTerrainTextureIcons(Event e, Runtime.Core.SelectionDatas.Group.Group group, List<Layer> paletteLayers, Rect windowRect)
		{
			...
			Rect virtualRect = SelectedWindowUtility.GetVirtualRect(windowRect, paletteLayers.Count, _protoIconWidth, _protoIconHeight);
			...
			for (int protoIndexForGUI = 0; protoIndexForGUI < paletteLayers.Count; protoIndexForGUI++)
			{
				...
				SetColorForIcon(paletteLayers[protoIndexForGUI].Selected, out var textColor, out var rectColor);
				SetIconInfoForTexture(paletteLayers[protoIndexForGUI].AssignedLayer, out var preview, out var name);
		...
		private static void HandleSelectLayer(int prototypeIndex, Runtime.Core.SelectionDatas.Group.Group group, Event e)
		{
			switch (e.type)
			{
