...
namespace VladislavTsurikov.MegaWorld.Editor.Common 
{
    public static class UndoEditor
    {
        ...
        {
	        if (!typeof(ToolWindow).IsAssignableFrom(toolType) && !typeof(MonoBehaviourTool).IsAssignableFrom(toolType))
	        {
		        return;
	        }
	        ...
	        if (toolType.GetAttribute<SupportedPrototypeTypesAttribute>().PrototypeTypes.Contains(selectedData.SelectedGroup.PrototypeType))
	        {
		        OnGUI();
	        }
        }
