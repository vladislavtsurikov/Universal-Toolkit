...
namespace VladislavTsurikov.MegaWorld.Editor.Common.Settings.FilterSettings.MaskFilterSystem.Elements 
{
    ...
    public class AspectFilterEditor : MaskFilterEditor
    {
        ...
        public override void OnGUI(Rect rect, int index) 
        {
            ...
            rect.y += EditorGUIUtility.singleLineHeight;
            _aspectFilter.EffectStrength = EditorGUI.Slider(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight), _strengthLabel, _aspectFilter.EffectStrength, 0.0f, 1.0f);
            rect.y += EditorGUIUtility.singleLineHeight;
            _aspectFilter.RemapCurve = EditorGUI.CurveField(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight), _curveLabel, _aspectFilter.RemapCurve);
        ...
        private static GUIContent _strengthLabel = EditorGUIUtility.TrTextContent("Strength", "Controls the strength of the masking effect.");
        private static GUIContent _curveLabel = EditorGUIUtility.TrTextContent("Remap Curve", "Remaps the concavity input before computing the final mask.");
    }
}
#endif