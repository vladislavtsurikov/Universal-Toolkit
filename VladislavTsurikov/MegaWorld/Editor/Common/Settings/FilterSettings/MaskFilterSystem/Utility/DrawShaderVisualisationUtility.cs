...
namespace VladislavTsurikov.MegaWorld.Editor.Common.Settings.FilterSettings.MaskFilterSystem.Utility
{
    public static class DrawShaderVisualisationUtility
    {
	    ...
	    public static void DrawMaskFilter(BoxArea boxArea, MaskFilterContext filterContext, float multiplyAlpha = 1)
    	{
            ...
            {                
	            VisualisationMaskFiltersPreference visualisationMaskFiltersPreference = PreferenceElementSingleton<VisualisationMaskFiltersPreference>.Instance;
    			...
    			blendMat.SetVector(_blendParams, new Vector4(0.0f, 0.0f, -(terrainPainterRenderHelper.BoxArea.Rotation * Mathf.Deg2Rad), 0.0f));
    			TerrainPaintUtility.SetupTerrainToolMaterialProperties(filterContext.HeightContext, terrainPainterRenderHelper.BrushTransform, blendMat);
    			Graphics.Blit(filterMaskRT, compRT, blendMat, 0);

    			RenderTexture.active = tmpRT;

                BrushTransform identityBrushTransform = TerrainPaintUtility.CalculateBrushTransform(terrainPainterRenderHelper.BoxArea.TerrainUnder, 
                    UnityTerrainUtility.GetTextureCoordFromUnityTerrain(terrainPainterRenderHelper.BoxArea.Center), terrainPainterRenderHelper.BoxArea.BoxSize, 0.0f);
                ...
                brushMaterial.SetInt(_alphaVisualisationType, (int)visualisationMaskFiltersPreference.AlphaVisualisationType);   
                brushMaterial.SetFloat(_alpha, visualisationMaskFiltersPreference.CustomAlpha * multiplyAlpha);   

                TerrainPaintUtility.SetupTerrainToolMaterialProperties(filterContext.HeightContext, identityBrushTransform, brushMaterial);
#if UNITY_2021_2_OR_NEWER
    			TerrainPaintUtilityEditor.DrawBrushPreview(filterContext.HeightContext, TerrainBrushPreviewMode.SourceRenderTexture, compRT, identityBrushTransform, brushMaterial, 0);
#else
