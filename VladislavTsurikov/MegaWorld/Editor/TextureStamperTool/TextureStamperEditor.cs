...
namespace VladislavTsurikov.MegaWorld.Editor.TextureStamperTool
{
    ...
    public class TextureStamperEditor : MonoBehaviourToolEditor
    {
		...
		public override void OnChangeGUIGroup(Group group)
		{
			...
			_textureStamper.AutoRespawnController.StartAutoRespawn(_textureStamper.StamperControllerSettings.DelayAutoRespawn, new SpawnStamperTool(_textureStamper));
		}
    	...
    	public static void AddStamper(MenuCommand menuCommand)
    	{
    		GameObject stamper = new GameObject("Texture Stamper")
            {
	            transform =
	            {
		            localScale = new Vector3(150, 150, 150)
	            }
            };
