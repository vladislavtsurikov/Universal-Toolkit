    ...
    public static class SelectionPrototypeUtility 
    {
        ...
        public static PlacedObjectPrototype GetSelectedProto(Group group)
        {
            ...
            if(group.PrototypeType == typeof(PrototypeGameObject))
            {
                ...
                {
                    proto = (PrototypeGameObject)GetRandomPrototype.GetMaxSuccessProto(group.GetAllSelectedPrototypes());
                }
                else
                {
            ...
            else
            {
                ...
                if(settings.RandomSelectPrototype)
                {
                    proto = (PrototypeTerrainObject)GetRandomPrototype.GetMaxSuccessProto(group.GetAllSelectedPrototypes());
                }
                else
                {
        ...
        {
            int precisionUnit = GetPrecisionUnit(group);
            ...
            ActiveObjectController.PlacedObjectData = PlaceObjectUtility.TryToPlace(group, (PlacedObjectPrototype)group.PrototypeList[precisionUnit], mouseMove.Raycast);
        }
        ...
        private static int GetPrecisionUnit(Group group)
        {
            ...
            if(group.PrototypeType == typeof(PrototypeGameObject))
            {
                for (int i = 0; i < group.PrototypeList.Count; i++)
                {
                    PrototypeGameObject proto = (PrototypeGameObject)group.PrototypeList[i];
                    if(GameObjectUtility.IsSameGameObject(proto.Prefab, ActiveObjectController.PlacedObjectData.GameObject))
                    {
                        return i;
                    }
                }
            ...
            else
            {
                for (int i = 0; i < group.PrototypeList.Count; i++)
                {
                    PrototypeTerrainObject proto = (PrototypeTerrainObject)group.PrototypeList[i];
                    if(GameObjectUtility.IsSameGameObject(proto.Prefab, ActiveObjectController.PlacedObjectData.GameObject))
                    {
                        return i;
                    }
