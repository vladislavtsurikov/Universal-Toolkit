Shader "Hidden/MegaWorld/Image"
{
   ...
   /// <returns></returns>
        ...
        {
            if (ApproximatelyEqual(c1.r, c2.r, epsilon) && ApproximatelyEqual(c1.g, c2.g, epsilon) &&
                ApproximatelyEqual(c1.b, c2.b, epsilon))
            {
                return 0.0f;
            }
        ...
        float GetCol2(v2f i)
        {
            ...
            float col2 = 0;
            float4 imageMaskColor = tex2D(_ImageMaskTex, UVOffset);
            //smoothstep(0, sqrt(1) / 2, distance(UVOffset, float2(0.5f, 0.5f))));
        ...
        Pass
        {
            ...
            {
                float result = 1;
                float value = GetCol2(i);

                switch (_BlendMode)
                {
                case 0: //Multiply
                    {
                        result = UnpackHeightmap(tex2D(_InputTex, i.pcUV)) * value;
                ...
                case 3: //GreaterThan
                    {
                        result = UnpackHeightmap(tex2D(_InputTex, i.pcUV));
                        if (value > UnpackHeightmap(tex2D(_InputTex, i.pcUV)))
                        {
                            result = value;
                        }
