Shader "PaintTexture"
{
    ...
    SubShader
    {
        ...
        sampler2D _SourceAlphamapRenderTextureTex;
        ...
        Pass // 0 paint splat alphamap
        {
            ...
            float4 PaintSplatAlphamap(v2f i) : SV_Target
            {
                ...
                float filterStrength = UnpackHeightmap(tex2D(_FilterTex, i.pcUV));

                if (UnpackHeightmap(tex2D(_SourceAlphamapRenderTextureTex, i.pcUV)) + 0.1 >= filterStrength)
                {
                    filterStrength = 0;
                }
