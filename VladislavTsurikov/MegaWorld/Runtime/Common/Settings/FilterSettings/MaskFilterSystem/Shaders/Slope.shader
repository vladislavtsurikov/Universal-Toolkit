Shader "Hidden/MegaWorld/Slope"
{
    ...
    SubShader
    {
        ...
        float GetFalloffAddSlope(float slope, float minSlope, float maxSlope, float localMinAddSlopeFalloff,
                                 float localMaxAddSlopeFalloff)
        {
            if (slope > maxSlope)
            {
        ...
        Pass
        {
            ...
            float4 frag(v2f i) : SV_Target
            {
                ...
                case 1: //FalloffAdd
                    {
                        slopeValue = GetFalloffAddSlope(slopeAngle, _MinSlope, _MaxSlope, _MinAddSlopeFalloff,
                                                        _MaxAddSlopeFalloff);
                        break;
                    }
                default:
