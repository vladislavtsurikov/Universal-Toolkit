...
namespace VladislavTsurikov.RendererStack.Editor.Core.Windows
{
    public partial class RendererStackWindow 
    {
        protected override void OnGUI()
        {
            ...
            {
                CustomEditorGUILayout.HelpBox("There are several types of Renderer Stack Manager at the level, but there should be only one");
                CustomEditorGUILayout.HelpBox("Open the Scene Data Manager Window via Windows -> VladislavTsurikov -> Scene Data Manager. You need to remove the Renderer Stack Manager from the inactive scene");
            }
            else if (sceneDatas.Count == 0)
            {
                ...
                if (CustomEditorGUI.ClickButton(settingsRect, "Create Level Renders for the active scene", ButtonStyle.Add))
                {
