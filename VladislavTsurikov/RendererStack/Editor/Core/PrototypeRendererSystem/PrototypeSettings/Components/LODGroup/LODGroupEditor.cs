...
namespace VladislavTsurikov.RendererStack.Editor.Core.PrototypeRendererSystem.PrototypeSettings.LODGroup
{
    ...
    public class LODGroupEditor : PrototypeComponentEditor
    {
		...
		public override void OnGUI(Rect rect, int index)
		{
			...
			{
				CustomEditorGUI.WarningBox(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight),"Missing LOD Group in prefab.");
				rect.y += CustomEditorGUI.SingleLineHeight;
		        
				return;
			}

			_lodGroup.SetLODFade(Prototype.RenderModel, CustomEditorGUI.Toggle(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight), LODFade, _lodGroup.EnabledLODFade));
			rect.y += CustomEditorGUI.SingleLineHeight;
				
			if (_lodGroup.EnabledLODFade)
			{
				_lodGroup.LodFadeForLastLOD = CustomEditorGUI.Toggle(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight), LODFadeForLastLOD, _lodGroup.LodFadeForLastLOD); 
				rect.y += CustomEditorGUI.SingleLineHeight;
				_lodGroup.LodFadeTransitionDistance = CustomEditorGUI.Slider(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight), 
			...
			if (_lodGroup.EnabledLODFade)
			{
				CustomEditorGUI.HelpBox(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight),"Your shader must have the \"LOD_FADE_CROSSFADE\" keyword and also the LOD Fade algorithm in the shader. You also need the unity_LODFade variable.");
				rect.y += CustomEditorGUI.SingleLineHeight;
			}

			CustomEditorGUI.Label(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight),"Number of LODs: " + Prototype.RenderModel.LODs.Count);
			rect.y += CustomEditorGUI.SingleLineHeight;
			
			_lodGroup.LODDistanceRandomOffset = Mathf.Max(0, CustomEditorGUI.FloatField(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight), 
