...
namespace VladislavTsurikov.RendererStack.Editor.Core.PrototypeRendererSystem
{
    public class PrototypeRendererEditor : RendererEditor
    {
		...
		public override void OnGUI()
        {
			...
			{
				GUILayout.Space(3);
				if(CustomEditorGUILayout.ToggleButton("Prototypes", _prototypeSettingsType == PrototypeSettingsType.Prototypes, ButtonStyle.General))
				{
					_prototypeSettingsType = PrototypeSettingsType.Prototypes;
				}
				GUILayout.Space(3);
				if(CustomEditorGUILayout.ToggleButton("Scene Settings", _prototypeSettingsType == PrototypeSettingsType.ActiveScene, ButtonStyle.General))
				{
					_prototypeSettingsType = PrototypeSettingsType.ActiveScene;
				}
				GUILayout.Space(3);
				if(CustomEditorGUILayout.ToggleButton("Global Settings", _prototypeSettingsType == PrototypeSettingsType.GlobalSettings, ButtonStyle.General))
				{
		...
		{
			PrototypeRendererTarget.SelectionData.SelectedData.RefreshSelectedParameters(PrototypeRendererTarget.SelectionData);
		...
		{
			foreach (var log in selectedPrototype.PrototypeConsole.PrototypeLogList)
			{
				if(log.Error)
		...
		{
			_selectRenderModelDataFoldout = CustomEditorGUILayout.Foldout(_selectRenderModelDataFoldout, "Render Model Data");
			...
			if(_selectRenderModelDataFoldout)
			{
				...
				for (int lodIndex = 0; lodIndex < renderModel.LODs.Count; lodIndex++)
				{
					...
					{
						lod.Materials[matIndex] = (Material)CustomEditorGUILayout.ObjectField(new GUIContent("Material"), lod.Materials[matIndex], typeof(Material));
					}
				}
