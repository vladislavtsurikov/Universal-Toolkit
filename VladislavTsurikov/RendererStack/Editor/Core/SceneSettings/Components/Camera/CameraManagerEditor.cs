...
namespace VladislavTsurikov.RendererStack.Editor.Core.SceneSettings.Camera
{
    ...
    public class CameraManagerEditor : IMGUIElementEditor
    {
		...
		public override void OnEnable() 
		{
			...
			_reorderableList = new ReorderableList(_cameraManager.VirtualCameraList, typeof(VirtualCamera), true, false, true, false);
		...
		private void RemoveFilter(ReorderableList list)
        {
	        _cameraManager.RemoveCamera(list.index);
        }
		...
		private void DrawElementCb(Rect totalRect, int index, bool isActive, bool isFocused)
        {
	        ...
	        bool cameraEnabled = !virtualCamera.Ignored;
	        ...
	        {
		        string cameraName = virtualCamera.Camera.gameObject.name;
            ...
            if (virtualCamera.CameraType != CameraType.SceneView)
            {
	            bool temporaryActive = virtualCamera.Active;
	            
	            virtualCamera.FoldoutGUI = CustomEditorGUI.HeaderWithMenu(headerRect, foldoutText, virtualCamera.FoldoutGUI, ref temporaryActive, () => Menu(_cameraManager.VirtualCameraList[index], index));
		...
		private void DrawCameraSettings(Rect rect, VirtualCamera virtualCamera)
        {
	        if (virtualCamera.CameraType == CameraType.SceneView)
	        {
		        if (virtualCamera.Camera == null)
		        {
			        EditorGUI.HelpBox(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight), "The sceneview camera was not found.", MessageType.Info);
			        rect.y += CustomEditorGUI.SingleLineHeight;
		        }
		        else
		        {
			        EditorGUI.BeginDisabledGroup(true);
			        CustomEditorGUI.ObjectField(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight),new GUIContent("Camera"), virtualCamera.Camera, typeof(UnityEngine.Camera));
			        rect.y += CustomEditorGUI.SingleLineHeight;
			        EditorGUI.EndDisabledGroup();
			        
			        EditorGUI.HelpBox(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight), "The sceneview camera is used during Edit Mode.", MessageType.Info);
	        ...
	        {
		        virtualCamera.Camera = (UnityEngine.Camera)CustomEditorGUI.ObjectField(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight),new GUIContent("Camera"), virtualCamera.Camera, typeof(UnityEngine.Camera));
		        rect.y += CustomEditorGUI.SingleLineHeight;
	        }
	        else
	        {
        ...
        {
	        VirtualCamera newVirtualCamera = new VirtualCamera(virtualCamera.Camera);
