...
namespace VladislavTsurikov.RendererStack.Editor.Common.PrototypeSettings
{
    ...
    public class ShadowEditor : PrototypeComponentEditor
    {
        ...
        public override void OnGUI(Rect rect, int index)
        {
            ...
            {	
                _shadowSettings.ShadowDistance = CustomEditorGUI.FloatField(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight), ShadowDistance, _shadowSettings.ShadowDistance);
            ...
            {
                CustomEditorGUI.WarningBox(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight), "Shadows disabled in Global Settings.");
                rect.y += CustomEditorGUI.SingleLineHeight;
            }
        ...
        private void DrawLODShadowSettings(ref Rect rect)
        {
            ...
            {
                rect.x -= 5;
                _selectLODShadowFoldout = CustomEditorGUI.Foldout(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight), "LOD Shadow Settings", _selectLODShadowFoldout);
                rect.y += CustomEditorGUI.SingleLineHeight;
                ...
                if(_selectLODShadowFoldout)
                {
                    ...
                    for(int i = 0; i < Prototype.RenderModel.LODs.Count; i++)
                    {
                        int index = i * 4;
                        if (i >= 4)
                        {
                        ...
                        int lodIndex = (int)_shadowSettings.ShadowLODMap[index];
                        
                        _shadowSettings.ShadowLODMap[index] = CustomEditorGUI.Popup(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight), 
        ...
        public List<GUIContent> shadowLODs = new List<GUIContent> {
        ...
        public List<GUIContent> LODs = new List<GUIContent> {
