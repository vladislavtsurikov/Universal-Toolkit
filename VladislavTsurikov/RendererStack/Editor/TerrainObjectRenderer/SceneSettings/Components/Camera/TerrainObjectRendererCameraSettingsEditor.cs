...
namespace VladislavTsurikov.RendererStack.Editor.TerrainObjectRenderer.SceneSettings.Camera
{
    ...
    public class TerrainObjectRendererCameraSettingsEditor : ReorderableListComponentEditor
    {
        ...
        public override void OnGUI(Rect rect, int index)
        {
            ...
            TerrainObjectRendererCameraSettings.LodBias = CustomEditorGUI.Slider(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight), LODBias, TerrainObjectRendererCameraSettings.LodBias, 0.1f, 1);
            rect.y += CustomEditorGUI.SingleLineHeight;
            TerrainObjectRendererCameraSettings.CameraCullingMode = (CameraCullingMode)CustomEditorGUI.EnumPopup(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight), CameraCullingMode, TerrainObjectRendererCameraSettings.CameraCullingMode);
            rect.y += CustomEditorGUI.SingleLineHeight;
            ...
            if (cameraManager.IsMultipleCameras())
            {
                ...
                {
                    TerrainObjectRendererCameraSettings.EnableColliders = CustomEditorGUI.Toggle(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight), new GUIContent("Enable Colliders"), TerrainObjectRendererCameraSettings.EnableColliders);
                    rect.y += CustomEditorGUI.SingleLineHeight;
                }
            }
        }
