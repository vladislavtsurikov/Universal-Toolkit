...
namespace VladislavTsurikov.RendererStack.Runtime.TerrainObjectRenderer
{
    public partial class TerrainObjectRenderer
    {
        ...
        {
            Quality quality = (Quality)RendererStackManager.Instance.SceneComponentStack.GetElement(typeof(Quality));
        ...
        private void UpdateCameraRendererData(List<Cell> visibleCellList, int cameraIndex) 
        {
            ...
            {
                List<Cell> currentCellList = TerrainObjectRendererData.GetCellsWithInstances(visibleCellList, SelectionData.PrototypeList[protoIndex].ID, out var totalInstanceCount);
        ...
        private void UpdateGPUBuffer(SelectionData selectionData, List<Cell> instancedIndirectCellList, int totalInstanceCount, int protoIndex, int cameraIndex)
        {
            RendererStackManager rendererStackManager  = RendererStackManager.Instance;

            CameraManager cameraManager = (CameraManager)rendererStackManager.SceneComponentStack.GetElement(typeof(CameraManager));
            ...
            PrototypeTerrainObject proto = (PrototypeTerrainObject)selectionData.PrototypeList[protoIndex];
            ...
            GPUFrustumCullingID.DispatchGPUFrustumCulling(rendererStackManager, prototypeRenderData, proto, renderModel, virtualCamera, totalInstanceCount, threadGroupsFrustum);

            for (int lodIndex = 0; lodIndex < renderModel.LODs.Count; lodIndex++)
            {
                for (int n = 0; n < renderModel.LODs[lodIndex].Mesh.subMeshCount; n++)
                {
                    ComputeBuffer.CopyCount(prototypeRenderData.LODRenderDataList[lodIndex].PositionBuffer.ComputeBuffer, 
                        prototypeRenderData.LODRenderDataList[lodIndex].ArgsBufferMergedLODList[n].ComputeBuffer, sizeof(uint) * 1);
                    ComputeBuffer.CopyCount(prototypeRenderData.LODRenderDataList[lodIndex].PositionShadowBuffer.ComputeBuffer, 
                        prototypeRenderData.LODRenderDataList[lodIndex].ShadowArgsBufferMergedLODList[n].ComputeBuffer, sizeof(uint) * 1);
                }
            }
        }
    }
