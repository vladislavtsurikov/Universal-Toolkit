...
using Mesh = VladislavTsurikov.ColliderSystem.Runtime.Mesh.Mesh;
...
namespace VladislavTsurikov.RendererStack.Runtime.TerrainObjectRenderer.RendererData.ColliderSystem
{
    ...
    /// </summary>
    [ExecuteInEditMode]
    public class TerrainObjectRendererColliderDebug : MonoSingleton<TerrainObjectRendererColliderDebug>
    {
        ...
        [SerializeField]
        private Vector3 _overlapBoxSize = new Vector3(5, 5, 5);
        [SerializeField]
        ...
        private List<ColliderCell> _overlappedScenes = new List<ColliderCell>();

        public Color HitTriangleColor = Color.red;
        public Color NodeColor = Color.white;
        public Vector3 OverlapBoxSize { get => _overlapBoxSize;
            set => _overlapBoxSize = Vector3.Max(value, Vector3.one * 1e-5f);
        }
        public float OverlapSphereRadius { get => _overlapSphereRadius;
            set => _overlapSphereRadius = Mathf.Max(value, 1e-5f);
        }
        public float HitPointSize { get => _hitPointSize;
                        ...
                        case DebugObjectMode.ShowHitRaycastCells:
                        {
                            if(_objectRayHit != null)
                            {
                                if (_objectRayHit.MeshRayHit != null)
                                {
                                    ...
                                    editorMesh.DrawRaycast(HandleUtility.GUIPointToWorldRay(Event.current.mousePosition), _largeObjectCollider.GetMatrix(), NodeColor);
                                }
                            }
            ...
            if(_objectRayHit != null)
            {
                ...
                if (_objectRayHit.MeshRayHit != null)
                {
                    ...
                    if (editorMesh != null)
                    {
                        ...
                        var triangleVerts = editorMesh.GetTriangleVerts(_objectRayHit.MeshRayHit.TriangleIndex, _largeObjectCollider.GetMatrix());
                            ...
                            case DebugObjectMode.ShowOverlapObjects:
                            {
                                ...
                                if (_demoMode == OverlapMode.BoxOverlap)
                                {
                                    ...
                                    if (objectHit != null)
                                    {
                                        ...
                                        _overlappedObjects = TerrainObjectRendererAPI.OverlapBox(_overlapBoxCenter, _overlapBoxSize, Quaternion.Euler(_overlapBoxEuler), overlapObjecFilter);
                                    }
                                }
                                else if (_demoMode == OverlapMode.SphereOverlap)
                                {
                                    ...
                                    if (objectHit != null)
                                    {
                                        ...
                                        _overlappedObjects = TerrainObjectRendererAPI.OverlapSphere(_overlapSphereCenter, _overlapSphereRadius, overlapObjecFilter, true);
                                    }
                                }
                                break;
                            }
                            ...
                            case DebugBvhCellsMode.ShowOverlap:
                            {
                                ...
                                if (_demoMode == OverlapMode.BoxOverlap)
                                {
                                    ...
                                    if (objectHit != null)
                                    {
                                        // Perform the overlap test
                                        _overlapBoxCenter = objectHit.Point;
                                        _overlappedScenes = OverlapBvhCellsBox(_overlapBoxCenter, _overlapBoxSize, Quaternion.Euler(_overlapBoxEuler));
                                    }
                                }
                                else if (_demoMode == OverlapMode.SphereOverlap)
                                {
                                    ...
                                    {
                                        _overlapSphereCenter = objectHit.Point;
                                        _overlappedScenes = OverlapBvhCellsSphere(_overlapSphereCenter, _overlapSphereRadius);
                                    }
                                }
                                break;
                            }
        ...
        {
            List<ColliderCell> cells = new List<ColliderCell>();
            foreach (SceneDataManager sceneDataManager in SceneDataManagerUtility.GetAllSceneDataManager())
            {
                TerrainObjectRendererData instantRendererCollider = (TerrainObjectRendererData)sceneDataManager.SceneDataStack.GetElement(typeof(TerrainObjectRendererData));
                
                if(instantRendererCollider == null)
                    continue;

                foreach (var item in instantRendererCollider.BVHCellTree.OverlapCellsBox(boxCenter, boxSize, boxRotation))
                {
                    cells.AddRange(item.TerrainObjectRendererCollider.OverlapCellBox(boxCenter, boxSize, boxRotation, false));
                }
            }
        ...
        private List<ColliderCell> OverlapBvhCellsSphere(Vector3 sphereCenter, float sphereRadius)
        {
            ...
            foreach (SceneDataManager sceneDataManager in SceneDataManagerUtility.GetAllSceneDataManager())
            {
                ...
                foreach (var item in instantRendererCollider.BVHCellTree.OverlapCellsSphere(sphereCenter, sphereRadius))
                {
                    cells.AddRange(item.TerrainObjectRendererCollider.OverlapCellSphere(sphereCenter, sphereRadius, false));
                }
            }
        ...
        private void DrawOverlappedScenesVolumesGizmos()
        {
            // Set the color and then loop through each overlapped object and draw it
            GizmosEx.PushColor(_overlappedSolidColor);
            foreach (var item in _overlappedScenes)
            {
                AABB aabb = new AABB(item.Bounds);
            ...
            foreach (SceneDataManager sceneDataManager in SceneDataManagerUtility.GetAllSceneDataManager())
            {
                ...
                if(instantRendererCollider != null)
                {
                    foreach (Cell cell in instantRendererCollider.CellList)
                    {
                        foreach (ColliderCell colliderCell in cell.TerrainObjectRendererCollider.CellList)
                        {
                            foreach (PrototypeBVHObjectTree item in colliderCell.PrototypeBVHObjectTreeStack.PrototypeBVHObjectTreeList)
                            {
        ...
        public void DrawRaycast()
        {
            ...
            foreach (BVHNodeRayHit<Sector> node in SectorLayer.GetCurrentSectorLayers(Sectorize.Sectorize.GetSectorLayerTag())[0].ObjectBoundsBvhTree.DrawRaycast(ray, NodeColor))
            {
                ...
                if(instantRendererCollider != null)
                {
                    foreach (BVHNodeRayHit<Cell> item in instantRendererCollider.BVHCellTree.DrawRaycast(ray, NodeColor))
                    {
                        foreach (BVHNodeRayHit<ColliderCell> colliderCell in item.HitNode.Data.TerrainObjectRendererCollider.DrawRaycast(ray, NodeColor))
                        {
                            foreach (PrototypeBVHObjectTree proto in colliderCell.HitNode.Data.PrototypeBVHObjectTreeStack.PrototypeBVHObjectTreeList)
                            {
