    ...
    public class GameObjectConverter : Extension
    {
        ...
        public static void ConvertGameObjectToTerrainObjectRenderer()
        {
            ...
            for (int i = 0; i < sceneObjects.Length; i++)
            {
                ...
                PrototypeTerrainObject proto = (PrototypeTerrainObject)terrainObject.SelectionData.GetProto(prefab);

                if(proto != null)
                {
                    TerrainObjectRendererAPI.AddInstance(proto, sceneObjects[i].transform.position, sceneObjects[i].transform.localScale, sceneObjects[i].transform.rotation);
                }
            }
        }
        ...
        public void ConvertTerrainObjectRendererToGameObject()
        {
            ...
            {
                PlaceGameObject(instance.Instance.Proto, instance.Instance.Position, instance.Instance.Scale, instance.Instance.Rotation);
            }
        }

        private void PlaceGameObject(PrototypeTerrainObject proto, Vector3 position, Vector3 scaleFactor, Quaternion rotation)
        {
            GameObject go;
