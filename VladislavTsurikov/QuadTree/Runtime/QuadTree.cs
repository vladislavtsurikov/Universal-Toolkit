...
namespace VladislavTsurikov.QuadTree.Runtime
{
    public class QuadTree<T> where T : IHasRect
    {
        ...
        public void Insert(T item) => _root.Insert(item);
        ...
        public void Query(Rect area, List<T> results) => _root.Query(area, results);

        public void Query(Rect area, Func<T, bool> onItemFind) => _root.Query(area, onItemFind);

#if UNITY_EDITOR
        public void DrawAllCells() => _root.DrawCellRecurse();
#endif
    }
}
