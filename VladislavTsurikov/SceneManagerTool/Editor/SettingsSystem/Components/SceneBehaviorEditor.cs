...
namespace VladislavTsurikov.SceneManagerTool.Editor.SettingsSystem
{
    ...
    public class SceneBehaviorEditor : ReorderableListComponentEditor
    {
        ...
        {
            _sceneBehavior = (SceneBehavior)Target;
        }

        public override void OnGUI(Rect rect, int index)
        {
            _sceneBehavior.SceneCloseBehavior = (SceneCloseBehavior)CustomEditorGUI.EnumPopup(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight),
                new GUIContent("Scene Close Behavior"), _sceneBehavior.SceneCloseBehavior);
            
            rect.y += CustomEditorGUI.SingleLineHeight;
            
            _sceneBehavior.SceneOpenBehavior = (SceneOpenBehavior)CustomEditorGUI.EnumPopup(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight),
